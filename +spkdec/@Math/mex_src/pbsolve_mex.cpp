 ...
 * Required arguments:
 *   A_bands     [D x N] lower band of a [N x N] positive definite matrix
 ...
 * A is assumed to be symmetric positive semi-definite.
 *
 * Kevin Shan, 2019-04-04
 *============================================================================*/
...
void posBandSolve(ptrdiff_t N, ptrdiff_t p, ptrdiff_t m, mxArray *mx_A, mxArray *mx_X)
{
    ...
    char uplo = 'L';            // This is the lower triangle of A
    ptrdiff_t nSubDiag = p-1;   // Number of subdiagonals in A
    ...
    pbsv(&uplo, &N, &nSubDiag, &m, A, &p, X, &N, &info);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (!(AB_is_dbl || mxIsSingle(mx_AB)) || (N==0))
        mexErrMsgIdAndTxt(errId, "A_bands must be a [D x N] array of real numbers");
    // b = input 1
